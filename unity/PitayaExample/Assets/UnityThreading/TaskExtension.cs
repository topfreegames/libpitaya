...
namespace UnityThreading
{
	public static class TaskExtension
	{
		...
		{
			var enumerable = that as Task[] ?? that.ToArray();
			var remaining = enumerable.Length;
		...
		{
			var whenSucceeded = that as Task[] ?? that.ToArray();
			var remaining = whenSucceeded.Length;
			var syncRoot = new object();

			foreach (var task in whenSucceeded)
			{
				task.WhenSucceeded(() =>
				{
					lock (syncRoot)
		...
		public static IEnumerable<Task> WhenFailed(this IEnumerable<Task> that, Action action, DispatcherBase target)
		{
			var hasFailed = false;
			var syncRoot = new object();
			var whenFailed = that as Task[] ?? that.ToArray();
			foreach (var task in whenFailed)
			{
				task.WhenFailed(() =>
				{
					lock (syncRoot)
		...
		/// <returns>This task.</returns>
		private static Task OnResult(this Task task, Action<object> action, DispatcherBase target)
		{
		...
		/// <param name="target">The DispatcherBase to perform the action on.</param>
		/// <returns>This task.</returns>
		private static Task OnResult<T>(this Task task, Action<T> action, DispatcherBase target)
		{
		...
		/// <returns>This task.</returns>
		private static Task<T> OnResult<T>(this Task<T> task, Action<T> action, DispatcherBase actionTarget)
		{
		...
		/// <param name="followingTask">The task to start.</param>
		/// <returns>This task.</returns>
		private static Task Then(this Task that, Task followingTask)
		{
		...
		/// <param name="target">The DispatcherBase to start the following task on.</param>
		/// <returns>This task.</returns>
		private static Task Then(this Task that, Task followingTask, DispatcherBase target)
		{
			that.WhenFailed(() =>
			{
				followingTask.Abort();
		...
		/// <param name="that"></param>
		/// <param name="taskToWaitFor"></param>
		/// <returns>This task.</returns>
		...
		/// <param name="action">The action to start.</param>
		/// <returns>This Enumerable of Tasks.</returns>
		private static IEnumerable<Task> ContinueWhenAnyEnded(this IEnumerable<Task> tasks, Action<Task> action)
		{
			var syncRoot = new object();
			var done = false;
			var continueWhenAnyEnded = tasks as Task[] ?? tasks.ToArray();
			foreach (var task in continueWhenAnyEnded)
			{
				task.WhenEnded(t =>
				{
					lock (syncRoot)
		...
		/// <param name="action">The action to start.</param>
		/// <returns>This Enumerable of Tasks.</returns>
		private static IEnumerable<Task> ContinueWhenAllEnded(this IEnumerable<Task> tasks, Action<IEnumerable<Task>> action)
		{
			var continueWhenAllEnded = tasks as Task[] ?? tasks.ToArray();
			var count = continueWhenAllEnded.Length;
			...
			foreach (var task in continueWhenAllEnded)
			{
				task.WhenEnded(t =>
				{
					lock (syncRoot)
