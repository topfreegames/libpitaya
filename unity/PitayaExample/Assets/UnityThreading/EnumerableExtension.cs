...
namespace UnityThreading
{
    public static class EnumerableExtension
    {
        ...
        {
            return (IEnumerable<Task>)that.ParallelForEach(element => { action(element); return default(Task.Unit); }, target);
        }
        ...
        /// <param name="target">The TaskDistributor instance on which the operation should perform.</param>
        /// <returns>IEnumerable of created tasks.</returns>
        private static IEnumerable<Task<TResult>> ParallelForEach<TResult, T>(this IEnumerable<T> that, Func<T, TResult> action, TaskDistributor target)
        {
            return that.Select(tmp => Task.Create(() => action(tmp)).Run(target)).ToList();
        }
        ...
        /// <param name="target">The TaskDistributor instance on which the operation should perform.</param>
        /// <returns>IEnumerable of created tasks.</returns>
        private static IEnumerable<Task> SequentialForEach<T>(this IEnumerable<T> that, Action<T> action, TaskDistributor target)
        {
            return (IEnumerable<Task>)that.SequentialForEach(element => { action(element); return default(Task.Unit); }, target);
        }
        ...
        /// <param name="target">The TaskDistributor instance on which the operation should perform.</param>
        /// <returns>IEnumerable of created tasks.</returns>
        private static IEnumerable<Task<TResult>> SequentialForEach<TResult, T>(this IEnumerable<T> that, Func<T, TResult> action, TaskDistributor target)
        {
